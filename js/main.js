...
function initMouseParallax() {
  ...
  function animate() {
    ...
    // Apply transforms — CSS transform is additive with existing scroll transform
    if (heroBgImg) {
      const px = currentX * 1.5; // percentage of viewport
      const py = currentY * 1.0;
      heroBgImg.style.setProperty('--mouse-x', `${px}%`);
      heroBgImg.style.setProperty('--mouse-y', `${py}%`);
      // We use separate translate so it doesn't override the scroll parallax
      // The heroBg element gets the scroll parallax, heroBgImg gets the mouse
    ...
    if (logoSeal && !logoSeal.classList.contains('settled') && !logoSeal.classList.contains('settling')) {
      const px = currentX * 1.0;
      const py = currentY * 0.7;
      logoSeal.style.transform = `translate(${px}%, ${py}%)`;
    }

    if (heroContent) {
      const px = currentX * 0.5;
      const py = currentY * 0.35;
...
/**
 * Projects Gallery Narrative
 * Handles horizontal drag, swipe, and scroll-driven movement
 */
function initProjectsGallery() {
  ...
  // Momentum / Easing config
  const lerp = (start, end, amt) => (1 - amt) * start + amt * end;
  let currentPos = 0;
  let targetPos = 0;

  // Preload first two images
  const preloadImages = ['assets/images/project 1 a.jpg', 'assets/images/project 1 b.jpg'];
  ...
  const update = () => {
    ...
    // Bounds check to prevent excessive overscroll
    const maxScroll = track.scrollWidth - wrapper.clientWidth;
    if (targetPos > 0) targetPos = lerp(targetPos, 0, 0.1);
    if (targetPos < -maxScroll) targetPos = lerp(targetPos, -maxScroll, 0.1);

    if (Math.abs(targetPos - currentPos) > 0.1 || isDragging) {
      rafId = requestAnimationFrame(update);
    } else {
      rafId = null;
    }
  ...
  // Passive Wheel horizontal scroll support
